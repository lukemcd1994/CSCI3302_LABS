...
def main(path):
    ...
    for x, y in path[1:]:
      ...
      end_t = abs(rot_time / 2) + t0
      while rospy.Time.now().to_sec() < end_t:
      ...
      msg.data = [1.9, 1.9]
      publisher_motor.publish(msg)
      while rospy.Time.now().to_sec() - t0 < fow_time/2:
        publisher_sim.publish(Empty())
...
if __name__ == "__main__":
  ...
  n_wp = [wp[0]]
  #make double attempt to waypoint because variations
  for p in wp[1:]:
    n_wp += p, p
  main(n_wp)
